...
const { data,isLoading,isError,isRefetching } = getNewRequestDetail(String(masterRequestId));
...
const handleYesClick= async ()=>{
 await deleteRequest();
  ...
  if (deleteRequestResponse?.status === false) {
    await toast.error(deleteRequestResponse?.errors[0]);
  } else {
    await toast.success(t('request_delete_success'));
 router.back();
          ...
          <CardContent>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    className="card-subHeading"
                      color={'var(--text-gray-3)'}
                      >  {moment(detailsData?.maxRequestFulfillExpiryDateTime).format("HH:mm  M-DD-YYYY")}</Typography>
                    </Box>
                    <VectorButton
                ...
                <Box  my={2} >
                  {detailsData?.files?.map((data:any,index:number) => (
                    <SingleFileHandler 
                    key={index}
                    fileName={data}
                    />
            ...
            >
                ...
                <Button className={`button_filled !px-8`}>
                  ...
                  <Button onClick={()=>setModalOpen(true)} className={`button_filled
                    !bg-transparent
